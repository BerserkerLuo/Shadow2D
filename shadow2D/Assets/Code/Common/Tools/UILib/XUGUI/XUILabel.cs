    ...
    public class XUILabel : XUIObject, IXUILabel
    {
        ...
        private IEnumerator DoTweenValue(float end, float fTime, string strFormat, float delay)
        {
            ...
            while (fRatio < 1.0f)
            {
                ...
                strText = string.Format(strFormat, fValue);
                SetText(strText);
                m_fLastValue = fValue;
            ...
            strText = string.Format(strFormat, end);
            if (null != m_uiLabel)
                m_uiLabel.text = strText;
            m_fLastValue = end;
            m_coroutine = null;
        ...
        public FontStyle fontStyle
        {
            get
            {
                ...
                {
                    return this.m_uiLabel.fontStyle;
                }
            ...
            set
            {
                if (null != this.m_uiLabel)
                {
                    this.m_uiLabel.fontStyle = value;
                }
            }
        }
        ...
        {
            base.Init();
            m_uiLabel = GetComponent<Text>();
            if (null == m_uiLabel)
            {
                Debug.LogError("null == m_uiLabel:" + XUITool.GetHierarchy(gameObject));
            }
        ...
        public void TypeText(string text, float printDelay = -1, UnityAction finish = null)
        {
            ...
            m_uiLabel.DOKill();
            printDelay = printDelay > 0 ? printDelay : text.Length*0.05f;
            m_uiLabel.DOText(text, printDelay).SetEase(Ease.Linear).onComplete = () =>
            {
                if (finish != null)
                {
                    finish();
