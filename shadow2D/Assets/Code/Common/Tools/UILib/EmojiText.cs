...
public class EmojiText : Text
{
	...
	private static Dictionary<string,EmojiInfo> EmojiIndex = new Dictionary<string, EmojiInfo>();
    private static bool m_bHasInitEmoji = false;

    public static void Init(string strContent)
    { 
        EmojiIndex.Clear();
        m_bHasInitEmoji = true;
...
#if UNITY_EDITOR
        if (false == m_bHasInitEmoji)
        {
            StreamReader reader = File.OpenText("Assets/StreamingAssets/data/config/emoji.txt");
    ...
    /// 点击事件检测是否点击到超链接文本
    /// </summary>
    public string OnPointerClick(PointerEventData eventData)
    {
        Vector2 lp;
